...
{
    public sealed class Chr__Token : StringFunctionToken
    {
        public Chr__Token(CodeParser Parser, String Content) : base(Parser, Content) { }

        public override string GetStringResult(Basic BasicIntrepeter)
        {
            byte Input = 0;

            BasicIntrepeter.CodeParser.Expect(typeof(Chr__Token));

            if (BasicIntrepeter.CodeParser.CurrentToken.GetType().Equals(typeof(LeftParenToken)))
            {
                Input = (byte) System.Math.Floor(BasicIntrepeter.NumericModifier.Value());
            }
            else
            {
            ...
            return new String(new char[] { (char)Input });
